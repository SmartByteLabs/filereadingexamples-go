...
// TestBufioRead tests the BufioRead function
...
func BenchmarkTestCase1(b *testing.B) {
	fileName := "words_1.txt"
	fileContent, err := testfiles.TestFiles.Open("words_1.txt")
	if err != nil {
	...
	for i := 0; i < b.N; i++ {
		// Run the BufioRead Function b.N times
		s, err = BufioRead(fileContent)
		if err != nil {
			b.Fatalf("BufioRead Failed for %s: %v", case1, err)
		}
...
func BenchmarkTestCase3(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		s, err = BufioRead(fileContent)
		if err != nil {
			b.Fatalf("BufioRead Failed for %s: %v", case3, err)
		}
		result = s
